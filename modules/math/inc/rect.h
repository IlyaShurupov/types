...
using rectf = rect<alnf>;
using recti = rect<alni>;
...
public:
	...
	bool inside(Type x, Type y) const { return (pos.x < x&& pos.y < y&& pos.x + size.x > x&& pos.y + size.y > y); }

	inline vec2<Type> size_vec() const { return vec2<Type>(size.x, size.y); }

	inline vec2<Type> size_vecw() const { return vec2<Type>(size.x + pos.x, size.y + pos.y); }
	...
	inline bool above(const rect<Type>& rect) const { return (pos.y + size.y < rect.pos.y); }
	inline bool bellow(const rect<Type>& rect) const { return (rect.pos.y + rect.size.y < pos.y); }
	inline bool right(const rect<Type>& rect) const { return (pos.x + size.x < rect.pos.x); }
	inline bool left(const rect<Type>& rect) const { return (rect.pos.x + rect.size.x < pos.x); }
